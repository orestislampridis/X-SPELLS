...
from keras.layers import Dense, LSTM, Conv1D, MaxPooling1D, Dropout, SpatialDropout1D, Conv2D, Activation
...
with open('LIME_po_DNN.csv', mode='w', newline='') as file:
...
exp = explainer.explain_instance(X_test[idx], loaded_model.predict_proba, num_features=10)
print('Document id: %d' % idx)
print('Probability(positive) =', loaded_model.predict_proba([X_test[idx]])[0, 1])
print('True class: %s' % class_names[y_test[idx]])
